...
class Candidate:
    ...
    alias_context_score: float = 0.0
    local_phase_score: float = 0.0
...
def _score_alias_v2(cands, ref_pre, srch_pre, cfg):
    # STEP 2 & 3: Periodic Alias Grouping
    period_x = 13.73
    period_y = 38.31
    ...
    # Identify the top classical candidate family
    best_c = cands[0]
    
    # We will compute alias_context_score for all candidates that are aliases of best_c
    # or just for all candidates to be safe, but apply it.
    ...
    for c in cands:
        # Check if it's a periodic alias of the best candidate
        dx = abs(c.x - best_c.x)
        dy = abs(c.y - best_c.y)
        
        nx = dx / period_x if period_x > 0 else 0
        ny = dy / period_y if period_y > 0 else 0
        
        is_alias_x = abs(nx - round(nx)) < tol
        is_alias_y = abs(ny - round(ny)) < tol
        
        c.alias_context_score = 0.0
        c.local_phase_score = 0.0
        c.geom_score = 0.0
        
        if is_alias_x and is_alias_y:
            # STEP 4, 5, 6: Contextual Discrimination & Multi-scale Context
            ctx_scores = []
            ph_scores = []
            
            for factor in [1.2, 1.5, 2.0]:
                nh = int(rh * factor); nw = int(rw * factor)
                if nh >= sh or nw >= sw: continue
                
                # Extract search context
                x0, y0 = max(0, c.x - nw//2), max(0, c.y - nh//2)
                x1, y1 = min(sw, x0 + nw), min(sh, y0 + nh)
                if (x1 - x0) < nw or (y1 - y0) < nh: continue
                s_patch = s_gray[y0:y0+nh, x0:x0+nw]
                
                # Extrapolate reference (synthetic context)
                pad_top = (nh - rh) // 2; pad_bottom = nh - rh - pad_top
                pad_left = (nw - rw) // 2; pad_right = nw - rw - pad_left
                r_ext = cv2.copyMakeBorder(r_gray, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_REFLECT)
                
                # Score similarity
                resp = cv2.matchTemplate(s_patch, r_ext, cv2.TM_CCOEFF_NORMED)
                ctx_scores.append(float(resp.max()))
                
                # Local phase correlation
                win = np.outer(np.hanning(nh), np.hanning(nw)).astype(np.float32)
                R = np.fft.fft2(r_ext * win)
                P = np.fft.fft2(s_patch * win)
                cross = R * np.conj(P)
                norm = cross / (np.abs(cross) + 1e-9)
                pc_map = np.abs(np.fft.ifft2(norm)).astype(np.float32)
                mean_v = float(pc_map.mean())
                if mean_v > 1e-12:
                    ph_scores.append(float(np.clip(pc_map.max() / (mean_v * max(nh,nw)) / 10.0, 0, 1)))
            
            if ctx_scores:
                c.alias_context_score = sum(ctx_scores) / len(ctx_scores)
            if ph_scores:
                c.local_phase_score = sum(ph_scores) / len(ph_scores)
...
def localize(ref_img: np.ndarray, search_img: np.ndarray, cfg: dict = None, arch: str = None) -> dict:
    ...
    if is_finfet:
        ...
        # EXPERIMENTAL PERIODIC V2 PATH
        cands = _score_alias_v2(cands, ref_pre, srch_pre, cfg)
        for c in cands:
            v2_score = 0.5 * c.alias_context_score + 0.5 * c.local_phase_score
            # We add a small bump to final_score to break ties between periodic aliases
            # The maximum bump is around 0.01 to ensure we don't overwhelm the classical
            # score differences of non-aliases.
            c.final_score += 0.01 * v2_score
            
        # Re-rank based on updated final score
        cands.sort(key=lambda c: c.final_score, reverse=True)
